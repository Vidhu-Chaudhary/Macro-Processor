...
FILE *f1, *f2, *f3, *f4;
char label[100][0], code[100][0], operand[100][0], du_label[100][0], du_code[100][0], du_operand[100][0], ala1[100][0], temp[100][0];
	...
	if ( mdlc == 0 )
	{
		...
		{ 
			if ( operand[i] == '#' )
			{
				...
				k = k + 1;
				strcpy(operand[i ++][0],s.data[s.sp + k + 1]);
				operand[i][0] = '\0';
				printf("\noperand %s\n",operand);
			}
			else
...
void PREPARE_ALA2 ()
{
	...
	while ( operand[i] != '\0' )
	{ 
		strcpy(s.data[s.sp + n + 1],operand[i ++]);
		n ++;
		//printf ("\ns.data[%d] = %c",s.sp + n + 1,s.data[s.sp + n + 1]);
		if (operand[i] == ',')
			i ++;
	}
}
